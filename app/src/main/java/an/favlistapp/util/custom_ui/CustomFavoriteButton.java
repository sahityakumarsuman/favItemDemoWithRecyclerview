package an.favlistapp.util.custom_ui;
...
public class CustomFavoriteButton extends ImageView {
    ...
    public CustomFavoriteButton(Context context) {
        super(context);
        init(context, null);
    }

    public CustomFavoriteButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CustomFavoriteButton(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = getTypedArray(context, attributeSet, R.styleable.CustomFavoriteButton);
        if (attr != null) {
            try {
                mButtonSize = Utils.dpToPx(
                        attr.getInt(R.styleable.CustomFavoriteButton_mfb_size, DEFAULT_BUTTON_SIZE), getResources());
                mAnimateFavorite = attr.getBoolean(R.styleable.CustomFavoriteButton_mfb_animate_favorite,
                        mAnimateFavorite);
                mAnimateUnfavorite =
                        attr.getBoolean(R.styleable.CustomFavoriteButton_mfb_animate_unfavorite,
                                mAnimateUnfavorite);
                mPadding = Utils.dpToPx(
                        attr.getInt(R.styleable.CustomFavoriteButton_mfb_padding, DEFAULT_PADDING),
                        getResources());
                if (attr.getResourceId(R.styleable.CustomFavoriteButton_mfb_favorite_image, 0) != 0
                        && attr.getResourceId(R.styleable.CustomFavoriteButton_mfb_not_favorite_image, 0)
                        != 0) {
                    mFavoriteResource =
                            attr.getResourceId(R.styleable.CustomFavoriteButton_mfb_favorite_image,
                                    FAVORITE_STAR_BLACK);
                    mNotFavoriteResource =
                            attr.getResourceId(R.styleable.CustomFavoriteButton_mfb_not_favorite_image,
                                    FAVORITE_STAR_BORDER_BLACK);
                } else {
                    setTheme(attr.getInt(R.styleable.CustomFavoriteButton_mfb_color, STYLE_BLACK),
                            attr.getInt(R.styleable.CustomFavoriteButton_mfb_type, STYLE_STAR));
                }

                mRotationDuration = attr.getInt(R.styleable.CustomFavoriteButton_mfb_rotation_duration,
                        mRotationDuration);
                mRotationAngle =
                        attr.getInt(R.styleable.CustomFavoriteButton_mfb_rotation_angle, mRotationAngle);
                mBounceDuration =
                        attr.getInt(R.styleable.CustomFavoriteButton_mfb_bounce_duration, mBounceDuration);
            } finally {
                attr.recycle();
            }
        }
    ...
    private void animateButton(boolean toFavorite) {
        ...
        animatorSet.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                if (mOnFavoriteAnimationEndListener != null) {
                    mOnFavoriteAnimationEndListener.onAnimationEnd(CustomFavoriteButton.this, mFavorite);
                }
            }
        });
    ...
    public static final class Builder {
        ...
        public CustomFavoriteButton create() {
            CustomFavoriteButton materialFavoriteButton = new CustomFavoriteButton(context);
